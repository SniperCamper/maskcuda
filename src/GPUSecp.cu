...
__device__ void _PointMultiSecp256k1(uint64_t* rx, uint64_t* ry, uint16_t* privKey, uint8_t* gTableX, uint8_t* gTableY, uint8_t* shared_gTableX, uint8_t* shared_gTableY) {
    int chunk = 0;
    ...
    for (; chunk < NUM_GTABLE_CHUNK; chunk++) {
      if (privKey[chunk] > 0) {
        ...
        if (shared_gTableX && shared_gTableY && index < 1024) {
            memcpy(rx, shared_gTableX + index, SIZE_GTABLE_POINT);
            memcpy(ry, shared_gTableY + index, SIZE_GTABLE_POINT);
        } else {
    ...
    for (; chunk < NUM_GTABLE_CHUNK; chunk++) {
      if (privKey[chunk] > 0) {
        ...
        int index = (CHUNK_FIRST_ELEMENT[chunk] + (privKey[chunk] - 1)) * SIZE_GTABLE_POINT;
        if (shared_gTableX && shared_gTableY && index < 1024) {
            memcpy(gx, shared_gTableX + index, SIZE_GTABLE_POINT);
            memcpy(gy, shared_gTableY + index, SIZE_GTABLE_POINT);
        } else {
...
__global__ void computePublicKeyKernel(uint64_t* privateKey, uint8_t* gTableX, uint8_t* gTableY, uint64_t* outputX, uint64_t* outputY) {
    ...
    _PointMultiSecp256k1(qx, qy, privKeyChunks, gTableX, gTableY, nullptr, nullptr);
